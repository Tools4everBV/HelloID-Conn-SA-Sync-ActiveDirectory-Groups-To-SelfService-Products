...
# Only removes Local groups (synced groups from e.g. AD or Entra ID are never removed)
...
######################################################################################
# Source Data Selection - Which AD groups to sync
######################################################################################
# Active Directory Group Properties to Retrieve
# REQUIRED: "objectGUID" (unique identifier) and "name" (used in product/group names)
# Common: objectGUID, name, samAccountName, description, distinguishedName, managedBy, etc.
# Full list: https://learn.microsoft.com/en-us/powershell/module/activedirectory/get-adgroup?view=windowsserver2025-ps
$adGroupPropertiesToRetrieve = @(
    "objectGUID" # REQUIRED: (unique identifier)
    , "name"
    , "samAccountName"
    , "description"
    , "distinguishedName"
...
# Filter which AD groups to sync (optional)
# Examples:
# - All groups: $null
# - By name: "name -like `"App-*`" -or name -like `"*-App`""
$adGroupsFilter = $null

# Organizational Units to search for AD groups
# Specify one or more OUs where the groups are located
# Examples:
# - All OUs: $null
# - Specific OUs: @("OU=Groups,DC=domain,DC=local", "OU=Apps,OU=Groups,DC=domain,DC=local")
$adGroupsOUs = $null
######################################################################################
...
$productIdentifierPrefix = "APPGRP"
...
######################################################################################
# This function generates the product configuration for a source object (e.g. group/mailbox)
...
function New-HelloIDProductConfiguration {
    ...
    return @{
        ...
        # Example: "Sales Team"
        Name                       = "AD group $($SourceObject.name)"
        ...
        Description                = "Access to the AD group $($SourceObject.name)"
        ...
        onApprove                  = @(
            ...
            [PSCustomObject]@{
                ...
                scriptVariableName = "addADUserToADGroupScript" # use variable name as string reference to avoid cluttering configuration with script content
                runInCloud         = $false
                agentPoolName      = "" # If left empty, uses default agent pool
            }
        )
        ...
        onReturn                   = @(
            ...
            [PSCustomObject]@{
                ...
                scriptVariableName = "removeADUserFromADGroupScript" # use variable name as string reference to avoid cluttering configuration with script content
                runInCloud         = $false
                agentPoolName      = "" # If left empty, uses default agent pool
            }
        )
...
# Browse icons at: https://fontawesome.com/v5/search?m=free
# Common examples: "envelope", "folder", "briefcase", "users", "building", "inbox"
$categoryFaIcon = "windows"  # Font Awesome icon name (e.g., "windows", "group")
...
#region Add AD user to AD Group script
...
`$adGroupCorrelationField = 'objectGUID'
`$adGroupCorrelationValue = [Guid]::New((`$product.code.replace("$productIdentifierPrefix","")))
...
$adUserCorrelationField = 'UserPrincipalName'
$adUserCorrelationValue = $requestedFor.userName
...
try {
    ...
    $actionMessage = "querying AD user where [$($adUserCorrelationField)] = [$($adUserCorrelationValue)]"
    ...
    $queryADUserSplatParams = @{
        Filter      = "$adUserCorrelationField -eq `"$adUserCorrelationValue`""
        ErrorAction = "Stop"
    }
    ...
    Write-Information "Queried AD user where [$($adUserCorrelationField)] = [$($adUserCorrelationValue)]. Name: [$($adUser.Name)], ObjectGUID: [$($adUser.ObjectGUID)], SID: [$($adUser.SID)]"

    # Query AD group (to use object in further actions)
    $actionMessage = "querying AD group where [$($adGroupCorrelationField)] = [$($adGroupCorrelationValue)]"
    ...
    $queryADGroupSplatParams = @{
        Filter      = "$adGroupCorrelationField -eq `"$adGroupCorrelationValue`""
        ErrorAction = "Stop"
    }
    ...
    Write-Information "Queried AD group where [$($adGroupCorrelationField)] = [$($adGroupCorrelationValue)]. Name: [$($adGroup.Name)], ObjectGUID: [$($adGroup.ObjectGUID)], SID: [$($adGroup.SID)]"

    # Add AD user to AD group
    $actionMessage = "adding AD user [$($adUser.Name) ($($adUser.ObjectGUID))] to AD group [$($adGroup.Name) ($($adGroup.ObjectGUID))]"
    ...
    $addADGroupMemberSplatParams = @{
        Identity    = $adGroup.ObjectGUID
        Members     = $adUser.ObjectGUID 
        PassThru    = $true
        Confirm     = $false
        ErrorAction = "Stop"
    }
    ...
    $Log = @{
        ...
        Message           = "Successfully added AD user [$($adUser.Name) ($($adUser.ObjectGUID))] to AD group [$($adGroup.Name) ($($adGroup.ObjectGUID))]" # required (free format text) 
        IsError           = $false # optional. Elastic reporting purposes only. (default = $false. $true = Executed action returned an error) 
        TargetDisplayName = $adUser.Name # optional (free format text)
        TargetIdentifier  = $adUser.ObjectGUID # optional (free format text)
    }
    #send result back  
...
try {
    ...
    # Remove AD user from AD group
    $actionMessage = "removing AD user [$($adUser.Name) ($($adUser.ObjectGUID))] from AD group [$($adGroup.Name) ($($adGroup.ObjectGUID))]"
    ...
    $Log = @{
        ...
        System            = "ActiveDirectory" # optional (free format text) 
        Message           = "Successfully removed AD user [$($adUser.Name) ($($adUser.ObjectGUID))] from AD group [$($adGroup.Name) ($($adGroup.ObjectGUID))]" # required (free format text) 
        IsError           = $false # optional. Elastic reporting purposes only. (default = $false. $true = Executed action returned an error) 
        TargetDisplayName = $adUser.Name # optional (free format text)
        TargetIdentifier  = $adUser.ObjectGUID # optional (free format text)
    }
...
# AD actions
try {
    Write-StatusMessage -Event Information -Message "-----------[AD]-----------"

    # Import ActiveDirectory module
    $actionMessage = "importing ActiveDirectory module"
    $importModule = Import-Module -Name "ActiveDirectory" -ErrorAction Stop

    # Get AD groups
    $adGroups = [System.Collections.ArrayList]@()
    
    # Determine filter - if empty or null, get all groups
    $effectiveFilter = if ([string]::IsNullOrWhiteSpace($ADGroupsFilter)) { "*" } else { $ADGroupsFilter }
    
    # Build query parameters
    $adQuerySplatParams = @{
        Filter     = $effectiveFilter
        Properties = $adGroupPropertiesToRetrieve
    }

    # Query based on OUs configuration
    if ([string]::IsNullOrWhiteSpace($ADGroupsOUs) -or ($ADGroupsOUs | Measure-Object).Count -eq 0) {
        # No OUs specified - query all OUs
        $actionMessage = "querying AD groups that match filter [$effectiveFilter] from all OUs"
        Write-StatusMessage -Event Information -Message "Querying AD groups that match filter [$effectiveFilter] from all OUs"
        
        $adGroupsResult = Get-ADGroup @adQuerySplatParams | Select-Object $adGroupPropertiesToRetrieve
        
        if ($adGroupsResult -is [array]) {
            [void]$adGroups.AddRange($adGroupsResult)
        }
        elseif ($null -ne $adGroupsResult) {
            [void]$adGroups.Add($adGroupsResult)
        }
        
        if ($verboseLogging -eq $true) {
            Write-Verbose "Successfully queried AD groups that match filter [$effectiveFilter] from all OUs. Result count: $(($adGroupsResult | Measure-Object).Count)"
        }
    }
    else {
        # OUs specified - query each OU
        foreach ($ADGroupsOU in $ADGroupsOUs) {
            $actionMessage = "querying AD groups that match filter [$effectiveFilter] in OU [$ADGroupsOU]"
            Write-StatusMessage -Event Information -Message "Querying AD groups that match filter [$effectiveFilter] in OU [$ADGroupsOU]"
            
            $adGroupsInOU = Get-ADGroup @adQuerySplatParams -SearchBase $ADGroupsOU -SearchScope Subtree | Select-Object $adGroupPropertiesToRetrieve
            
            if ($adGroupsInOU -is [array]) {
                [void]$adGroups.AddRange($adGroupsInOU)
            }
            elseif ($null -ne $adGroupsInOU) {
                [void]$adGroups.Add($adGroupsInOU)
            }
            
            if ($verboseLogging -eq $true) {
                Write-Verbose "Successfully queried AD groups that match filter [$effectiveFilter] in OU [$ADGroupsOU]. Result count: $(($adGroupsInOU | Measure-Object).Count)"
            }
        }
    }

    Write-StatusMessage -Event Success -Message "Successfully queried AD groups that match filter [$effectiveFilter]. Result count: $(($adGroups | Measure-Object).Count)"
    ...
    foreach ($adGroup in $adGroups) {
        # IMPORTANT: Add displayName property mapping - DO NOT REMOVE!
        # AD groups use "name" property, but the rest of this script expects "displayName" property (like Entra ID groups)
        # This mapping ensures compatibility with the product configuration function and all downstream logic
        $adGroup | Add-Member -MemberType NoteProperty -Name "displayName" -Value $adGroup.name -Force
        
        [void]$sourceObjectsInScope.Add($adGroup)
    }
